...
> 🎯 **释放存储空间** - 存储价格暴涨，空间寸土寸金  
> 👨‍👩‍👧 **保留有意义的片段** - 只留下有家人、有人物的珍贵画面  
> ✂️ **智能压缩** - 裁剪静止画面，合并碎片文件，大幅减少文件数量  
> 🔒 **本地 AI 处理** - 数据不出本机，隐私安全无忧

自动检测监控视频中的人物，提取有人的片段并合并输出。使用 YOLOv8 本地视觉模型 + FFmpeg 多线程解码，高效利用多核 CPU。
...
Pipeline 根据 CPU 核心数自动配置参数（针对 FFmpeg 解码瓶颈优化）：

| CPU 核心数 | I/O Workers | Detectors | Batch Size | 解码线程数 |
|------------|-------------|-----------|------------|------------|
| 1-2 核     | 1           | 1         | 16         | 2          |
| 3-4 核     | 2           | 2         | 32         | 2          |
| 5-8 核     | 2           | 4         | 48         | 4          |
...
> **说明:** 较少的 I/O Worker 配合更多的解码线程，可以提升高分辨率视频（如 2304x1296）的处理吞吐量，因为 FFmpeg 解码是主要瓶颈。
